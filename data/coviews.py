...
print ("(#1) Map to (user_id, item_id) ")
for key,value in output:
    print("Pages: user ID %s and item_id %s" % (key, value))
print ("done printing pages")

users_with_list = pages.groupByKey() # Group item_ids to user_id: (user_id, list of item ids they clicked on) (#2)
output = users_with_list.mapValues(list).collect()
print ("(#2)Group item_ids to user_id: (user_id, list of item ids they clicked on)")
for key,value in output:
    print("Pages: user ID %s and items clicked %s" % (key, value))
print ("done printing users_with_list")
...
output = users_with_pairs.collect()
print ("(#3) Transform into (user_id, (item1, item2) where item1 and item2 are pairs of items the user clicked on")
for key,value in output:
    print("Pages: user ID %s and pair %s" % (key, value))
print ("done printing users_with_pairs")
...
output = pairs_with_users.mapValues(list).collect()
print ("(#4) Transform into ((item1, item2), list of user1, user2 etc) where users are all the ones who co-clicked (item1, item2)")
for key,value in output:
    print("Pages: pairs of items clicked ID %s and users list %s" % (key, value))
print ("done printing pairs_with_users")
...
output = pairs_with_users_count.collect()
print ("(#5) Transform into ((item1, item2), count of distinct users who co-clicked (item1, item2)")
for key,value in output:
    print("Pages: pair %s and count %s" % (key, value))
print ("done printing pairs_with_users_count")
...
print ("(#6) Filter out any results where less than 3 users co-clicked the same pair of items")
for key,value in output:
